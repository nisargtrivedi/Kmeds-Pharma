...
public class My_order_detailActivity extends CommonAppCompatActivity {
    ...
    private TextView tv_date, tv_type, tv_charge, tv_price, tv_cancel,tvReorder;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout ll_offer_discount = (LinearLayout) findViewById(R.id.ll_order_detail_offer_discount);
        TextView tv_offer_discount = (TextView) findViewById(R.id.tv_order_detail_offer_discount);
        ...
        String offer_discount = getIntent().getStringExtra("offer_discount");

        getSupportActionBar().setTitle(getResources().getString(R.string.order_no) + sale_id);

        Double finalPrice = Double.valueOf(total_amount) + Double.valueOf(delivery_charge);

        if (!offer_discount.isEmpty() && !offer_discount.equalsIgnoreCase("0")) {
            finalPrice = finalPrice - Double.valueOf(offer_discount);
            ll_offer_discount.setVisibility(View.VISIBLE);
            tv_offer_discount.setText(offer_discount);
        } else {
            ll_offer_discount.setVisibility(View.GONE);
        }

        tv_date.setText(on_date);
        tv_type.setText(payment_type);
        tv_charge.setText(delivery_charge);
        tv_price.setText(String.format("%.2f", finalPrice));
        ...
        CommonAsyTask task = new CommonAsyTask(params,
            ...
            public void VResponce(String response) {
                ...
                try {
                    ...
                    Toast.makeText(My_order_detailActivity.this,response,Toast.LENGTH_LONG).show();
